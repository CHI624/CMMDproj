...
$form1 = $_SESSION['form_res'] ?? [];
$form2 = $_SESSION['form_res1'] ?? [];
...
Job: {$form_res['job']}
Job Description: {$form_res['job_descr']}
...
Risk Score: {$form_res1['risk_score']}
Severity Score: {$form_res1['severity_score']}
Combined Hazards: {$form_res1['combined_hazards']}
Final Risk Level: {$form_res1['final_risk']}
...
<main class="chat-interface">
  ...
  <div style="background:rgb(117, 117, 117); color: #f0f0f0; padding: 1rem; border-radius: 8px; margin-bottom: 1rem;">
    <p><strong>Severity Score:</strong> <?= htmlspecialchars($form_res1['severity_score']) ?></p>
    <p><strong>Risk Score:</strong> <?= htmlspecialchars($form_res1['risk_score']) ?></p>
    <p><strong>Final Risk Level:</strong> <?= htmlspecialchars($form_res1['final_risk']) ?></p>
  </div>
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const dbResponse = await fetch('ARMOR_COND_REARCHER.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
